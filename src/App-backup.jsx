...
/**
 * Elmcrest Communication & Motivation Compass – React App (JS version)
...
/*************** NEW: COMMUNICATION × MOTIVATION COMBOS ***********/
...
export default function App() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setPostStatus(
        "⚠️ We couldn't confirm saving to the Sheet, but your results are still visible above."
      );
    }
  }
  ...
  function buildSummaryText() {
    ...
    if (primaryCommInfo) {
      lines.push(`About your primary style (${primaryCommInfo.title}):`);
      lines.push(primaryCommInfo.desc);
      lines.push("");
      lines.push("Tips for stretching this style in your Elmcrest role:");
      primaryCommInfo.tips.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
    }

    if (commCombo) {
      lines.push(`Primary + Secondary combination: ${commCombo.label}`);
      lines.push(commCombo.summary);
      lines.push("");
      lines.push("How this often shows up with others:");
      commCombo.howOthersExperience.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
      lines.push("Stretch ideas for your communication:");
      commCombo.stretch.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
    }
    ...
    if (primaryMotivInfo) {
      lines.push(`About your primary driver (${primaryMotivInfo.title}):`);
      lines.push(primaryMotivInfo.desc);
      lines.push("");
      lines.push("This tends to be fueled by:");
      primaryMotivInfo.fuels.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
    }

    if (motivCombo) {
      lines.push(`Primary + Secondary combination: ${motivCombo.label}`);
      lines.push(motivCombo.summary);
      lines.push("");
      lines.push("What this tends to mean:");
      motivCombo.whatThisMeans.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
      lines.push("Ideas to better support your motivation:");
      motivCombo.tryThis.forEach((t) => lines.push(`• ${t}`));
      lines.push("");
    }
    ...
    if (commMotivCombo) {
      ...
      lines.push("----------------------------------------------");
      lines.push(`${commMotivCombo.label} – ${commMotivCombo.summary}`);
      lines.push("");
      lines.push("Key strengths in your Elmcrest role:");
      commMotivCombo.strengths.forEach((s) => lines.push(`• ${s}`));
      lines.push("");
      lines.push("Important watch-outs to be mindful of:");
      commMotivCombo.watchOuts.forEach((w) => lines.push(`• ${w}`));
      lines.push("");
      lines.push("Support ideas and reflection prompts:");
      commMotivCombo.supportIdeas.forEach((s) => lines.push(`• ${s}`));
      lines.push("");
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300">
              For Elmcrest Program Supervisors • Shared with your manager
            </p>
          </div>
          <Pill label="Public" />
        </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 print:py-4">
        <div className="grid md:grid-cols-3 gap-6 print:grid-cols-1">
          ...
          <aside className="md:col-span-1 bg-white dark:bg-slate-800 rounded-2xl shadow p-6 print:shadow-none print:border print:border-slate-200">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
              These update as you answer. Final results appear after submission
              and are saved to Google Sheets if configured.
            </p>
            ...
            <div className="mt-6 space-y-4">
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                ...
                <ul className="list-disc pl-5 text-sm text-slate-700 dark:text-slate-200 space-y-1">
                  {COMM_INFO[totals.primaryComm].tips.map((t, i) => (
                    <li key={i}>{t}</li>
                  ))}
                </ul>
              </div>
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                ...
                <div className="text-sm text-slate-700 dark:text-slate-200">
                  ...
                  {MOTIV_INFO[totals.primaryMotiv].fuels.join(", ")}
                </div>
              </div>
            </div>
          </aside>
            ...
            <section className="mt-10 grid md:grid-cols-2 gap-6 print:grid-cols-2">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6 border border-slate-200 dark:border-slate-700 print:shadow-none">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300 mb-3">
                  ...
                  work together in real conversations with Shift Supervisors,
                  YDPs, youth, families, and your own supervisor.
                </p>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6 border border-slate-200 dark:border-slate-700 print:shadow-none">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300 mb-3">
                  ...
                  top two drivers can shape what you need from work at Elmcrest.
                </p>
            ...
            {commMotivCombo && (
              <section className="mt-8 bg-white dark:bg-slate-800 rounded-2xl shadow p-6 border border-slate-200 dark:border-slate-700 print:shadow-none">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300 mb-3">
                  ...
                  your main source of motivation work together in your role as a
                  Program Supervisor.
                </p>
                <p className="text-sm mb-2">
                  <strong>
                ...
                <div className="mt-2">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">
                    Reflection questions you can use in supervision or self-work:
                  </p>
